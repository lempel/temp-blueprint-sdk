...
public class ForkHandler extends AbstractHandler {
	...
	public ForkHandler(final XmlConfig config) {
		super(config);
	}
	 ...
	 * 
	 * @see
	 * lempel.blueprint.app.launcher.AbstractHandler#launch(java.lang.String[])
	 */
					...
					public void run() {
						while (runFlag) {
							try {
								...
								if (buffer.length > 0) {
									input.read(buffer);
									LOGGER.print(new String(one)
											+ new String(buffer, CharsetUtil.getDefaultEncoding()));
								}
							} catch (IOException e) {
